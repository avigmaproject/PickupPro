...
export default class TeamRight extends Component {
  constructor(props) {
    super(props);
    this.state = {
      listPlayer: [],
  ...
  componentDidMount = async () => {
    this.setState(
      ...
      () => console.log("captainname", this.props.route.params.captainname)
    );
    this.getAccessToken();
    ...
    await selectedplayer(data, this.state.token)
      .then((res) => {
        ...
        res[0].map((i) => {
          ...
          if (i.User_IsSelected && i.Team_PL_Id === 2) {
            console.log("Team_PL_Id", i.Team_PL_Id);
  ...
  createPlayerJSON = async () => {
    if (this.state.listPlayer.length !== 0) {
      if (this.state.listPlayer.length == 1) {
        alert("Please select team with 5 player maximum");
      } else {
        let output = [];
        let tmp;
        ...
        let data = {
          Type: 1,
          PT_GT_PkeyID: this.state.gameid,
          PT_Team_PL_Id: 2, //Left 1,right 2
          PlayerDataList: PlayerDataList,
        };
        console.log("datarightteam", data);
        await postrightteamdata(data, this.state.token)
          .then((res) => {
      ...
      alert("You can select maximum 5 + 1 player in a team");
    }
  };
  select = async (id) => {
    console.log(id);
  ...
  render() {
    ...
    >
      ...
      >
        Players shooting on the basket to your right
      </Text>
    </View> */}
